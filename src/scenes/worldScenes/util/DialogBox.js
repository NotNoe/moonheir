export default class DialogBoxScene extends Phaser.Scene {
    constructor() {
    ...
    init(data) {
        ...
        // @ts-ignore
        this.input.keyboard.addKey("e", true, true).on("down", event => {
            if(this.beingAnimated) return;
            this.scene.resume(data.scene_name);
            this.scene.stop("DialogBoxScene");
        })
        this.setText(data.text, true);
    }
